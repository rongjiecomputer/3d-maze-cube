...
export class HollowMaze3D {
  ...
  private isSurface(x: number, y: number, z: number): boolean {
  ...
  private getNeighbors(cell: Cell): { cell: Cell; dir: keyof Cell['walls']; oppDir: keyof Cell['walls'] }[] {
    ...
    const addNeighbor = (nx: number, ny: number, nz: number, dir: keyof Cell['walls'], oppDir: keyof Cell['walls']) => {
      if (nx >= 0 && nx < this.size && ny >= 0 && ny < this.size && nz >= 0 && nz < this.size) {
        if (this.isSurface(nx, ny, nz)) {
          neighbors.push({ cell: this.grid[nx][ny][nz], dir, oppDir });
        }
      }
    };

    addNeighbor(x - 1, y, z, 'left', 'right');
    addNeighbor(x + 1, y, z, 'right', 'left');
    addNeighbor(x, y - 1, z, 'bottom', 'top');
    addNeighbor(x, y + 1, z, 'top', 'bottom');
    addNeighbor(x, y, z - 1, 'back', 'front');
    addNeighbor(x, y, z + 1, 'front', 'back');
